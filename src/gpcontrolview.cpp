...
void GPControlView::newGPControl()
{
    ...
    if (ui->deterministRadioButton->isChecked()) {
        gpControl->setNextCarMode(DETERMINIST);
    } else if (ui->sequentialRadioButton->isChecked()) {
        gpControl->setNextCarMode(SEQUENTIAL);
    }
...
void GPControlView::on_randomRadioButton_toggled(bool toggle)
{
    if (toggle) {
        gpControl->setNextCarMode(RANDOM);
    }
}
void GPControlView::on_deterministRadioButton_toggled(bool toggle)
{
    if (toggle) {
        gpControl->setNextCarMode(DETERMINIST);
    }
}
void GPControlView::on_sequentialRadioButton_toggled(bool toggle)
{
    if (toggle) {
        gpControl->setNextCarMode(SEQUENTIAL);
    }
...
void GPControlView::gpControl_carMoved()
{
    ...
    QString res = tr("Driver %1: move to (%2,%3) new velocity is (%4,%5), %6 boosts left %7 carburant")
                  .arg(curCar + 1)
...
void GPControlView::gpControl_carMovedWithBoost()
{
    emit mapChanged(gpControl);
    int curCar = gpControl->getCurrentCar();
    QString res = tr("Driver %1 boost: move to (%2,%3) new velocity is (%4,%5), %6 boosts left")
                  .arg(curCar + 1)
                  .arg(gpControl->getPosCarX(curCar))
                  .arg(gpControl->getPosCarY(curCar))
                  .arg(gpControl->getVelCarX(curCar))
